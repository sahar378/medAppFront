...
const CommandeForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (type === 'materiel') {
          setProduits(produitsData.filter(p => p.categorie.idCategorie === 1));
        } else if (type === 'medicament') {
          setProduits(produitsData.filter(p => p.categorie.idCategorie === 2));
        }
  ...
  const handleProduitChange = async (selectedOption) => {
    ...
    try {
      ...
      Swal.fire(
        'Erreur',
        `Impossible de récupérer les fournisseurs avec prix pour ce produit. Détails: ${
          error.response?.status ? `Statut ${error.response.status}` : error.message
        }`,
        'error'
      );
    }
  ...
  const handleFournisseurChange = async (fournisseurId) => {
    ...
    if (fournisseur.statut !== 0) {
      Swal.fire(
        'Erreur',
        `Le fournisseur "${fournisseur.nom}" est inactif et ne peut pas être utilisé pour un bon de commande.`,
        'error'
      );
      setSelectedFournisseurId('');
      return;
      ...
      <div className="content-wrapper">
        <div className="content-header">
          <div className="container-fluid">
            <h1 className="m-0">
              {bonCommandeId ? 'Modifier' : 'Créer'} un bon de commande - {type === 'materiel' ? 'Matériels' : 'Médicaments'}
            </h1>
          </div>
        </div>
                    ...
                    >
                      ...
                      {fournisseurs.map(f => (
                        <option
                          ...
                          style={f.statut !== 0 ? { color: 'red' } : { color: 'black' }}
                          disabled={f.statut !== 0}
                        >
